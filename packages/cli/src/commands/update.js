    ...
    .action((taskId, opts) => {
      ...
      if (opts.status) {
        if (!STATUSES.includes(opts.status)) {
          console.error(
            chalk.red(`Invalid status "${opts.status}". Valid: ${STATUSES.join(", ")}`),
          );
          process.exit(1);
        }
      ...
      if (opts.evaluation) {
        if (!EVALUATIONS.includes(opts.evaluation)) {
          console.error(
            chalk.red(`Invalid evaluation "${opts.evaluation}". Valid: ${EVALUATIONS.join(", ")}`),
          );
          process.exit(1);
        }
      ...
      if (opts.priority) {
        if (!PRIORITIES.includes(opts.priority)) {
          console.error(
            chalk.red(`Invalid priority "${opts.priority}". Valid: ${PRIORITIES.join(", ")}`),
          );
          process.exit(1);
        }
      ...
      if (!changed) {
        console.log(
          chalk.yellow(
            "No changes specified. Use --status, --evaluation, --priority, --notes, or --deps.",
          ),
        );
        return;
      }
